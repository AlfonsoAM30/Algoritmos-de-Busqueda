...
import BusquedasLocales.Operator2OPT;
...
public class MemeticoSinReps {
    ...
    public MemeticoSinReps(Instancia instancia, int tamanioPoblacion, int iteraciones,
    ...
    public Solucion run() {
        ...
        for (int i = 0; i < this.iteraciones; i++) {
            ...
            BusquedaLocal busquedaLocal = new Operator2OPT();
            // Búsqueda local
            if (i % 10 == 0) {
                // Convertir O a List para aplicar la búsqueda local
                List<Solucion> listaO = new ArrayList<>(O);
                for (Solucion s : listaO) {
                    busquedaLocal.run(s, instancia);
                }
                // Convertir de nuevo a Set
                O = new HashSet<>(listaO);
                RellenarPoblaciones.rellenarPoblacion(O, poblacionAnterior, tamanioPoblacion, instancia);
            }
